          ...
          parallel {
            stage('tester A') {
              steps {
                ...
                sh "npm run test:record:${BROWSER_1}:parallel --spec ${SPEC_1} --ci-build-id ${env.BUILD_ID}"
              }
            }
            ...
            stage('tester B') {
              steps {
                echo "Running build ${env.BUILD_ID}"
                sh "npm run test:record:${BROWSER_2}:parallel --spec ${SPEC_2} --ci-build-id ${env.BUILD_ID}"
              }
            }
          }
